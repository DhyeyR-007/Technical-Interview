def decompress (width, height, data):
    trit_to_Char = [' ', '.', '@']
    ...
    for row in range(height):
      row__ = ""
      for col in range((width+4)//5):
        if ind < len(data):
          byte = data[ind]
          trits = decode_byte(byte)
          for t in trits:
            if len(row__) < width:
              row__ += trit_to_Char[t]
          ind += 1
          
      decompress_row.append(row__[:60])
      
    decompress_img = "\n".join(decompress_row)
    return decompress_img
...
decompressed_img = decompress(width, height, data)
